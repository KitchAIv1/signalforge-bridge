...
import { useAlphaOmegaToxicCrackSkipSwitch } from '@/hooks/useAlphaOmegaToxicCrackSkipSwitch';

/** Config-only remote for alpha_omega_toxic_crack_skip_enabled. No skip math. */
export function ToxicCrackSkipToggle() {
  const { enabled, toggleError, isSaving, handleToggle } =
    useAlphaOmegaToxicCrackSkipSwitch();
    ...
    <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-amber-500/20 pt-3">
      <div>
        <p className="text-xs font-medium text-slate-700 dark:text-slate-200">
          Toxic crack skip — {enabled ? 'ON' : 'OFF'}
        </p>
        <p
          className="mt-0.5 text-[11px] text-slate-500 dark:text-slate-400"
          title="Skips weak cracks on mushy pre-tape. Place-only; streak untouched."
        >
          Skips weak cracks on mushy pre-tape. Place-only; streak untouched.
        </p>
      </div>
      <button
        type="button"
