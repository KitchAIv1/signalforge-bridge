import type { RebuildSessionRow } from '@/lib/rebuildShadowAggregates';
import { omegaPct, omegaR2 } from '@/lib/omegaShadowFormat';

interface RebuildShadowSessionTableProps {
  rows: RebuildSessionRow[];
}

export function RebuildShadowSessionTable({ rows }: RebuildShadowSessionTableProps) {
  return (
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="text-slate-400 border-b border-slate-100">
              ...
              <th className="text-right pb-2 pr-3">Bar-1 rate</th>
              <th className="text-right pb-2">Avg P&L R</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.session} className="border-b border-slate-50 text-slate-700">
                <td className="py-2 pr-3">{r.session}</td>
                ...
                <td className="text-right py-2 pr-3">
                  {r.bar1Rate !== null ? omegaPct(r.bar1Rate) : '—'}
                </td>
