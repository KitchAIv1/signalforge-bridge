...
function resolveBanner(
  ...
  if (liveArmed) {
    return { text: `LIVE ${rule}`, active: true };
  }
  return { text: `Research: would ${side} (${rule})`, active: false };
}
...
export function PdlSweepLiveConditions({ todayRow, liveArmed, paused }: LivePanelProps) {
  ...
  const side = conditions ? pdlLiveSideFromConditions(conditions) : null;

  const bannerClass = banner.active
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4 space-y-4 dark:border-slate-700 dark:bg-slate-900">
      <div className="text-xs font-medium text-slate-500 uppercase tracking-wide">
        ...
        {side ? ` — intended ${side.toUpperCase()}` : ''}
      </div>
      ...
      <div className="text-xs text-slate-500 dark:text-slate-400">
        SHORT if all✗ or all✓ · else LONG · AMD:{' '}
        {todayRow?.amd_outcome_tag ?? 'pending'} | Live engine: pdl_window
